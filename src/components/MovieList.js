...
const MovieList = ({ type }) => {
  ...
  const watchList = movieStore.movies.filter((movie) => movie.name.toUpperCase().includes(query.toUpperCase())).map((movie) => (movie)).filter((movie) => movie.watched === false).map((movie) => (
...
<h4>{movie.name}</h4>
    ...
    const watchedList = movieStore.movies.filter((movie) => movie.name.toUpperCase().includes(query.toUpperCase())).map((movie) => (movie)).filter((movie) => movie.watched === true).map((movie) => (
      <li className="list-group-item d-flex justify-content-between align-items-center">
        <h4>{movie.name}</h4>
        <div className="btn-group btn-group-toggle" data-toggle="buttons">
          <Lable className="btn active">
  ...
  return type === true ? (
    <>
      <SearchBar label="Movies To Watch with Yousif  🔍" setQuery={setQuery}/>
        ...
        {watchList.length <= 0 ? <WarningMessage>No Movies Found.... Yousif Says Hi ;)</WarningMessage>:null }
    ...
    <>
      <SearchBar label="Movies Watched with Anas  🔎" setQuery={setQuery}/>
        ...
        {watchedList.length <= 0 ? <WarningMessage>No Movies Found.... Anas Says Hi ;)</WarningMessage>:null }
